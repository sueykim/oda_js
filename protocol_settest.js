...
function initPg()
  ...
  function loadTest()
    ...
    function loadTestSuccess(responseText, args)
      {
        function ss(r, args)
        {
        var ss = extract_CDATA( r )
        alert(ss)
        loadSets( myLevel, ss )
        }
      //==> debug    revert back to load from db
      ...
      alert(LC)
	    g_XX.XML_HTTP_GET( LC,ss, function(){}, [] )
	    /*
      function spin()
      {
    ...
    spin()
      ...
      //==> revert next line for db loading
      //loadSets( myLevel, ss )
      }// loadTestSuccess
...
//  if (checkMac())
//    w.g_clickGuardWidth = w.g_clickGuardWidth*0.9;
