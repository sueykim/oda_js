...
function form_class()
  ...
  var lname_form = new RegExp(/^[a-zA-Z0-9\s-]+$/);
...
//----- End of synchronization --- //
        ...
        //if( g_XX.get_responseText().length < 5 )
          ...
          for (var q=0, rL=document.start_oda.Tlang.length; q<rL; q++)
            if(document.start_oda.Tlang[q].checked)
              ...
              document.storage.tlang.value = document.start_oda.Tlang[q].value;
              break;
              }
         ...
         //add "select your start level
         for (var s=0, slvlL=document.start_oda.sl_lvl.length; s<slvlL; s++)
            if(document.start_oda.sl_lvl[s].checked)
              {
              tz = true
              var slvl_value = document.start_oda.sl_lvl[s].value > 0 ? document.start_oda.sl_lvl[s].value : 3
              instr.push(slvl_value)
              break;
              }
          if( tx && ty && tz )
            g_FM.post_results_to_server(document.start_oda,instr)
          else
            alert("Please make sure that you have selected a language, a skill, and the starting level before proceeding.")
          break;
          //}
        //else
  ...
  this.load_form = function( f )
    ...
    g_XX.XML_HTTP_GET( url, load_form_success, onfail, [f])
    //load_form_success(f)
    }//end load form 

  function load_form_success(responseText, args)
    {
    //alert('load_form_success:\n')
    var f = parseInt(args[0])
    var ss = extract_CDATA(responseText) //extract_CDATA( g_XX.get_responseText() )
  ...
  this.prep_linx = function( P )
    ...
    //for "survey" button
    with ( new foundation_gif_btn_classB( 'survey', xid('linkplaceL') ) )
      do_click = new Function( "callSurveypage()" )
  ...
  this.post_results_to_server = function( x, instr )
    ...
    function scensOnSuccess(responseText, args)
      {
      var scen = args[0]
      if ('scen_00' ==  scen)
        {
        //responseStr = g_XX.get_responseText().replace(/true/ig,'1').replace(/false/ig,'0')
        responseStr = responseText.replace(/true/ig,'1').replace(/false/ig,'0')
        if ( responseStr )
          {
      ...
      else if ('scen_01' == scen)
        {
        //responseStr = g_XX.get_responseText() //-- Turn this on to allow direct login with no email correspondence beforehand. ----
        responseStr = responseText //-- Turn this on to allow direct login with no email correspondence beforehand. ----
        if(responseStr == '99')
      ...
      else if ('scen_03' ==  scen)
        {
        responseStr = g_XX.get_responseText()
        }// end scen_03

      else if ('scen_06' ==  scen)
        {
        //responseStr = g_XX.get_responseText().replace(/true/ig,'1').replace(/false/ig,'0')
        responseStr = responseText.replace(/true/ig,'1').replace(/false/ig,'0')
        if (responseStr)
      ...
      else if ('scen_07' ==  scen)
        {
        var sid = parseInt(responseText)
      ...
      else if ('scen_08' ==  scen)
        {
        //alert('scens on success scen_08')
        ...
        //alert('scens on success scen_08 end of it')
         /**/
        }// end scen_08
    ...
    if( scen_00 )
      {
      scen = 'scen_00'
      g_XX.post_form( g_SERVER + g_login, getURLString(0), scensOnSuccess, onfail, [scen] )
      }
    else if( scen_01 )
      {
      scen = 'scen_01'
      var bbb =  getURLString(1)
      g_XX.post_form( g_SERVER + g_login, bbb , scensOnSuccess , onfail, [scen])
      }
    else if( scen_02 )
      {
      scen = 'scen_02'
      ...
      {
      scen = 'scen_03'
      g_XX.post_form( g_SERVER + g_login, getURLString(3) , scensOnSuccess, onfail, [scen])
      }
    else if ( scen_06 )
      {
      scen = 'scen_06'
      ...
      g_XX.post_form( g_SERVER + g_login, the_str , scensOnSuccess, onfail, [scen])
      }
    else if( scen_07 )
      {
      scen = 'scen_07'
      ...
      CGI_o.store('StrtLevel', instr[2])               //sets starting level
      //var sid = createSessionId([instr[0], instr[1], document.storage.uid.value, CGI_r.last, CGI_r.frst])
      createSessionId([instr[0], instr[1], document.storage.uid.value, CGI_r.last, CGI_r.frst], scensOnSuccess, [scen])
      }
    else if( scen_08 )
      {
      function cap( lcs )
      ...
      //var output = g_WS.postForm('GetProfilesByUserId', args)
      //alert('Post results to server_scen_08')
      g_WS.postForm('GetProfilesByUserId', args, scensOnSuccess, onfail, [scen])
...
/*      if (xid('id_popup_wrapper')) {
  ...
  //SK 20140703 revised takeODA -made two pages and add the NAV button in between
  //absolete takeODA has only 1 page
  function goto_takeODA_pg1()
    {
    xid('id_takeoda_pg1').style.display = 'inline'
    xid('id_takeoda_pg2').style.display = 'none'
    }
  function goto_takeODA_pg2()
    {
    xid('id_takeoda_pg1').style.display = 'none'
    xid('id_takeoda_pg2').style.display = 'inline'  
    }
  
  //SK 20140507 click behavior of new Survey button
  function callSurveypage()
    {
    var surveyId = document.storage.uid.value
    var surveyTlang = document.storage.tlang.value
    if (surveyTlang == '')
      {
    //  alert(surveyId)
      callWarning(surveyId, surveyTlang)
      }
    else
      window.open("oq2.html?uid=" + surveyId + "&tlang=" + surveyTlang, "_blank")
    }
  function callWarning(surveyId, surveyTlang)
    {
//    alert('surveyId from callPrompt: ' + surveyId)
/*    var cr = confirm("It appears you have NOT started the test yet. Are you sure you want to do the ODA User Survey?")
    if (cr == true)
      window.open("oq2.html?uid=" + surveyId + "&tlang=" + surveyTlang, "_blank")
*/
    var statesdemo =
      {
        ...
        {
          title: 'Warning!',
          html: 'It appears you have <u>NOT</u> started the test in this session yet. <br />Are you sure you want to continue?',
          buttons: { Cancel: -1, Continue: 1 },
          focus: 0,
          ...
          submit: function (e, v, m, f) {
            ...
            e.preventDefault();
            if (v == -1) $.prompt.close();
            if (v == 1) {$.prompt.close();window.open("oq2.html?uid=" + surveyId + "&tlang=" + surveyTlang, "_blank") };
          }
        }
      }
    $.prompt(statesdemo)
  ...
  function callLvlStartHelp()
	  {
		var statesdemo =
      {
        state0:
        {
          title: '<b>&nbsp;&nbsp;&nbsp;Starting Level Help</b>',
          html: 'Be conservative when selecting your starting level.<br /><br />For example, if you think you might be able to perform at level 2, we recommend that you start at level 1+. The system will adapt to your performance and move you up or down accordingly.<br /><br /> If you are not sure about your level, select "Don\'t know," and you will start at level 1+. ODA will adapt to your performance.  <br /><br />Starting levels are based on Interagency Language Roundtable (ILR) level Descriptions.',
          buttons: {  Close: 1 },
          focus: 0,
          //focus: "input[name='fname']",
          submit: function (e, v, m, f) {
	...
	function callWarningStatement()
  {
  //for warning statement
  var statesdemo =
      {
        state0:
        {
          title: 'Notification!',
          html: 'There will be a 3 minute scheduled maintenance outage for the Lingnet server, on Sunday September 21, 2014 between 6:00 a.m. and 6:30 a.m. PST. <br />Please be aware of this and adjust your test time accordingly. Thank you.',
          buttons: {  Continue: 1 },
          focus: 0,
          //focus: "input[name='fname']",
          submit: function (e, v, m, f) {
...
function initSys()
  ...
  if (arguments.length > 0)
    {
    if ('NO_LOAD_FORM' == arguments[0])
       LOADFORM = false
    }
    
  //alert('initSys and caller:\n'+arguments.callee.caller.toString())
