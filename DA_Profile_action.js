    ...
    if( lvlW >= -1 && lvlC < 9 )
      {
        ...
        var pbs = classify(document, 'img', 'css_btn_sug')
     ...
     {
       xid( "sug" ).style.display = "none"
     }
    }
  
  function suggest()
  {
    window.location.href = sugLink 
  }
  //change ==>
...
function emailIt()
{
  ...
  var statesdemo =
  {
    ...
    {
      title: 'Warning!',
      html: 'Sending a link of this profile will enable the recipient to view all information contained in it. Are you sure you want to continue?',
    ...
    {
      title: 'Email',
      html: 'Please enter recipient\'s email address.<br /><input type="text" name="email" value=""><br />',
      buttons: { Cancel: -1, Send: 1 },
      //focus: ":input:first",
      submit: function (e, v, m, f) {
        e.preventDefault();
        if (v == -1) $.prompt.close();
        if (v == 1){ sendEmail(f.email);}
      }
    }
  }
  $.prompt(statesdemo)
...
{  
  //long message will get truncated in IE
  //var email = prompt("WARNING!\nSome text here to warn the user.\nEnter email address below to proceed \nor click cancel to cancel.", "email@company.com")
  if (email != '')
  {
    var body = "Greetings,\n\nI have completed a session on ODA. Click on the link below to view my diagnostic profile. If the link does not work, simply copy and paste the URL below to your browser address bar.\n"
    var link = "mailto:"+encodeURIComponent(email)+"?Subject="+encodeURI("ODA Profile")+"&body="+encodeURIComponent(body+window.location+"&view=false")
...
window.onload = function()
  ...
  var CGI = new CGI_input_class()
  var pbs = classify(document, 'img', 'css_btn_email')
  for (var w = 0, z = pbs.length; w < z; w++) {
    if (!CGI.view) {
      with (new foundation_gif_btn_class(pbs[w].id))
        do_click = emailIt
      ...
      xid(pbs[w].id).style.display = "none"
    }
  }
