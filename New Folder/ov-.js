...
function enableLC(zz)
  {
  xid("sk_02").disabled = (zz.value == "russian" || zz.value == "chinese") ? true : false //(zz.value == "korean"||zz.value == "arabic")
  }
...
function form_class()
  ...
  var lastPage = ''
  var ZZ     = null
  var E_form = new RegExp(/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*\.(\w{3}|(com|net|org|edu|int|mil|gov|arpa|biz|aero|name|coop|info|pro|museum))$/)
  var xmls = []
...
//----- End of synchronization --- //
    if ( (L_name.length > 0)&&(F_name.length > 0)&&(E_form.test(E_mail))&&(passes))
      condition = true
    return condition
    }
  ...
  this.post_results_to_server = function( x, instr )
    ...
    if( scen_00 )
      ...
      responseStr = g_XX.get_responseText()
      var stg = g_XX.get_responseText().split("&")
      var strg = stg.join(" ")
      xid("receiver").innerHTML = '<p>' + strg + '</p>'
      }
    else if( scen_09 )
      {
      scen = 'scen_09'
