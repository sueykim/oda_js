...
function enableLC(zz)
  {
  xid("sk_02").disabled = false //(zz.value == "russian") ? true : (zz.value == "korean"||zz.value == "arabic") || zz.value == "chinese"
  }
...
//----- End of synchronization --- //
  ...
  this.post_results_to_server = function( x, instr )
    ...
    if( scen_00 )
      ...
      function cap( lcs )
        {
        return lcs.substr(0,1).toUpperCase() + lcs.substr(1, lcs.length-1)
        }
      scen = 'scen_08'
      var CGI_r = new CGI_response( document.storage.usp.value )
      ...
      for( var q=0, r=RS.length; q<r; q++ )
        ...
        if( RS[q].indexOf("{") > -1 )
          {
          eval( "var E = " + RS[q] )
          SR.push( ( E.lang ) ? "<h3>" + cap( E.lang ) + "<\/h3>" : "<p><a href=\"" + E.h + "\" target=\"_blank\">" + E.t + "<\/a><\/p>" )
          }
        }
      xid("receiver").innerHTML = '<p>' + SR.join("")+ '</p>' //'<p>' + strg + '</p>'
      }
    else if( scen_09 )
      {
      scen = 'scen_09'
