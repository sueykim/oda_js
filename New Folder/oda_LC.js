...
function activateModBtn( m_id, typx )
  ...
  if(myConf == false)
    ...
    for( var w=0, z=mQL.length; w<z; w++ )
      ...
      //mQL[w].style.width = 385 + "px"
      //mQL[w].style.left  = 386 + "px"
      }
    xid(modAudId).style.display = "inline"
    //xid(modAudId).style.width = 385 + "px"
    //xid(modAudId).style.left =  0 + "px"   this
      ...
      if(typx != 'CI')
        ...
        for(var i=0; i <a.length; i++)
          ...
          if(a[i].className == "css_questItem" || a[i].className=="css_QuestionBoxLC")
            {
            a[i].disabled = true
            //a[i].style.width = '370px'
            }
          //if(a[i].className == "css_questionLC_css_ltrTxt" ||a[i].className == "css_questionLC_css_rtlTxt" ||a[i].className == "css_questionLC" ||a[i].className == "css_questionMiddle_css_ltrTxt" ||a[i].className == "css_questionMiddle_css_rtlTxt")
          //  a[i].style.width = '370px'
          }
        //for(var j=0, k=b.length ; j<k; j++ )
        //  {
          //if(b[j].className == "css_questItem"||b[j].className=="css_QuestionBoxLC")
          //  b[j].style.width = '740px' //'370px'       //370
          //if(b[j].className == "css_questionLC_css_ltrTxt" ||b[j].className == "css_questionLC_css_rtlTxt" ||b[j].className == "css_questionLC" ||b[j].className == "css_questionMiddle_css_ltrTxt" ||b[j].className == "css_questionMiddle_css_rtlTxt")
          //  b[j].style.width = '740px' //'370px'       //370
        //  }
        //if(typx == 'RB')    to gray out the all radio buttons
...
function showLtrs(id, n, m, L)
  ...
  if(myLook == false)
    ...
    if(my_conf)
      ...
      for (var i=0; i<m; i++)
        {
        xid('id_script' + shortId + i + '_0' ).style.fontFamily = L == 'arabic' ? "Arial" : "Verdana"
        xid('id_script' + shortId + i + '_0' ).style.textAlign  = L == 'arabic' ? "right" : "left"
        xid('id_script' + shortId + i + '_0' ).style.direction  = L == 'arabic' ? "rtl" : "ltr"
