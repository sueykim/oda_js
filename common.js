...
function onfail()
 ...
 alert('system failed!')
    ...
    function s( B )
      ...
      if (B.length > 0 )
        {
        xid('stat').style.display = 'block'
        }
      else
        {
        xid('stat').style.display = 'none'
        }
      xid('stat').innerHTML = B//xid('stat').innerHTML + '<br>' + B
      }
...
function XML_HTTP_class()
  ...
  */
  this.XML_HTTP_GET = function(urlIn, successF, failF, passedArgs)
    {
    ...
    s('')
    $.tries = 1
    execute_GET(urlIn, successF, failF, (passedArgs|| []))
    }
  execute_GET = function( url, success, fail, passedArgs )
    {
    //alert('get caller\n'+arguments.callee.caller.toString())
    //alert(success)
    $.method = 'get'
    $.lcl_url = url
    $.lcl_successF = success
    $.lcl_failF = fail
    $.lcl_passedArgs = passedArgs
    clearTimeout(timout)
  ...
  this.post_form = function (urlIn, dataIn, successF, failF, passedArgs) 
    {
    //alert('post caller\n'+arguments.callee.caller.toString())
    //alert('post_form'+arguments.callee.caller.toString())
    //alert('xml_http_post')
    //alert('post ' + $.jqxhr)
    //alert('post_form\nXML_HTTP_CLASS tries=' + $.tries)
    //alert('post_form'+arguments.callee.caller.toString())
    if ($.jqxhr != '')
      {
      $.jqxhr.abort()
    ...
    s('')
    $.tries = 1
    execute_POST(urlIn, dataIn, successF, failF, passedArgs)
    }
  execute_POST = function(url, datastring, success, fail, passedargs)
    {
    //alert('excute_post in xmlhttp class')
    $.method = 'post'
    $.lcl_url = url
    $.lcl_postData = datastring
    $.lcl_successF = success
    $.lcl_failF = fail
    $.lcl_passedArgs = passedargs
    clearTimeout(timout)

    var newSSS = preventCache(url, datastring)
    $.jqxhr = $.ajax({
      type: "POST",
...
//    alert(qs.join('&'))
    ...
    */
    function createSessionId(vals, onsuccessF, passedArgs)// lang, mod, uid, last name , frst name
      {
      function localSuccess(responseText, args)
        {
      ...
      //alert(args)
      //output = g_WS.postform('StartNewSession', args)     
      g_WS.postForm('StartNewSession', args, localSuccess, onfail, [])
  ...
  function webServiceClass() 
    ...
    this.postForm = function (webMethodName, args, onsuccessF, onfailF, passedArgs) 
      {
      //alert('postForm on web service')
      //alert(arguments.callee.caller.toString())
      function localOnSuccess(responseText, args)
        {
        //alert('local on success in postForm web service \n')
        //if (g_XX.get_responseText().toLowerCase().indexOf('<string') > -1) 
        if (responseText.toLowerCase().indexOf('<string') > -1) 
          {
          var doc = BuildXMLFromString(responseText.substring(responseText.toLowerCase().indexOf('<string')))
          }
        else 
          {
          //for debugging don't delete
          //doc.loadXML('<string>{"genstring:"'+myReq.responseText+'"}<\/string>')
          var doc = BuildXMLFromString('<string>{"genstring":"' + responseText.replace(/\r\n/g, "  ").replace(/\n/g, "  ").replace(/\r/g, "  ").replace(/&/g, ' ') + '"}<\/string>')
          }

        onsuccessF(doc, passedArgs)
        }//localOnSuccess
      ...
      g_XX.post_form( currURL, str, localOnSuccess, onfailF, [])
    ...
    this.logError2Master = function (errcode, errdata) 
      ...
      this.postForm('InsertODAError', errArgs, function(x, y){}, onfail, [])
      } // logError2Master
