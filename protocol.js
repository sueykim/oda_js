...
*/
    ...
    if( NN >= 0 && NN < LL )
      ...
      if( document.fm_track.skill.value == 'reading' )
        ...
        //==> cick guard
/*        if (currentItemType != 'HTO')
          {

          //==>
          //check if scrollbar is present
          var dv //classify(ZZ[NN], 'div', 'css_TextBoxRTL')
          if ((classify(ZZ[NN], 'div', 'css_TextBoxRTL')).length > 0)
            {
            dv = classify(ZZ[NN], 'div', 'css_TextBoxRTL');
            }
          else if ((classify(ZZ[NN], 'div', 'css_TextBoxRTL_r_dir')).length > 0)
            {
            dv = classify(ZZ[NN], 'div', 'css_TextBoxRTL_r_dir');
            }
          else if ((classify(ZZ[NN], 'div', 'css_TextBoxLTR')).length > 0)
            {
            dv = classify(ZZ[NN], 'div', 'css_TextBoxLTR');
            }
          else if ((classify(ZZ[NN], 'div', 'css_TextBoxLTR_r_dir')).length > 0)
            {
            dv = classify(ZZ[NN], 'div', 'css_TextBoxLTR_r_dir');
            }
          var clickGuardWidt =  g_clickGuardWidth
          var algn = isIn(lang, rtlLangs) ? "right" : "left"
          if (checkMac())
            algn="center"
          if ( dv && (dv.length > 0) )
            {
            if ((dv[0].offsetWidth - dv[0].clientWidth) > 0)
              {
              clickGuardWidt = g_clickGuardWidth - g_scrollBarWidth;
              }

						$('#clickGuard').css('width', clickGuardWidt)
            $('#clickGuard').css('text-align', algn)
						//$('#clickGuard').css(algn, 0)
						$('#clickGuard').show();
						}
					}
        else
...
*/
  ...
  this.post_results_to_server = function()
    ...
    function gradeSetSuccess(responseText, args)
	 ...
	 if (true)  //(parseInt(document.fm_track.UID.value) == 658)  //to test different level
        {
      ...
      if (!foundErr)
        ...
        //alert(p + ' - ' + p.value)
        alert('TXR: ' + TXR)
        levelChk = TXR.pop()//[6]  true//==>
        var UD   = TXR.pop()
        alert('levelChk: ' + levelChk + '    UD: ' + UD)
        ...
        if(levelChk)
          ...
          for( var i=0, j=setHistory.length; i<j; i++ ) //for( var i=toolow ? 1 : 0, j=setHistory.length; i<j; i++ )
            {
            //var shtory = setHistory[i]
            //for (var sh in shtory)
            //  for(var s in sh)
            //  alert( 'setHistory' + i + ' - ' + sh + ': ' + s + '-' + s )
            if( (setHistory[i][1][5].split('=')[1] >= levelSet && setHistory[i][1][5].split('=')[1]<= topSet) || setHistory[i][1][5].split('=')[1] == levelSet-1 )
...
function initPg()
  ...
  function loadTest()
    ...
    var sessionID = document.fm_track.sessionID //added by SK 20141017
    var LC

    function loadTestSuccess(responseText, args)
      {
      //==> debug
        var ss = extract_CDATA(responseText.getElementsByTagName('string')[0].childNodes[0].wholeText)
    ...
    //==>debug here
	  ...
	  var args = []
	  args.push(['dbName', 'oda2_'+lang+'_'+ (skill == 'listening' ? 'lc' : 'rc') ])
	  args.push(['sessionId', sessionID] )   //added by SK 20141017
	  g_WS.postForm('GetTest', args, loadTestSuccess, {}, [])
...
function tracer()
  ...
  this.append = function( xx, yy )//xx = currentSet, yy = test points
    {
    alert('myLevel: ' + myLevel)
    ...
    XX += 1
    if( ((xx == 3 )|| (xx == 5) || (xx == 7 ) || (xx == 9 )) && YY <= 66 && xx <= myLevel )
      ST = -2
    //else if( (( xx%2 == 1 && YY <= 66 ) || ( xx == 9 )) && myLevel < xx ) //to make the lvl 3 user go through test set 8
    else if( ((xx%2 == 1 && xx > myLevel)||(xx == 1)) && YY <= 66  )
      ST = -1
    //else if( (xx == 1 && YY > 66 && myLevel > 1) || (( xx%2 == 1 && xx < 9 && xx > 1 ) && YY > 66 && myLevel > xx))
    else if( ( xx%2 == 1 && xx < 9  ) && YY > 66 &&  xx < myLevel )
      ST = 1
    else if( ( xx%2 == 1 && xx < 9 ) && YY > 66 && xx >= myLevel)
      ST = 2
    else if( ( xx%2 == 0 && YY <= 66 ) || (xx == 9 && YY > 66) )
      {
      ST = 0
      LC = false
      }
    else if( xx%2 == 0 && XX <= 10 && YY > 66)
      {
      if (xx < myLevel-2)
        ST = 2
      else
...
function addScrollBarWidthFunctionTo$()
{
    (function ($)
    {
...
window.onload = function()
  ...
  initJQAjaxSettings()
  document.oncontextmenu = new Function("return false")  //this is one of the new ones
...
//  w.g_clickGuardWidth = $('#clickGuard').width();
//  w.g_scrollBarWidth = $.getScrollbarWidth();
//  if (checkMac())
//    w.g_clickGuardWidth = w.g_clickGuardWidth*0.8;
 // alert(w.g_clickGuardWidth)  
  }
