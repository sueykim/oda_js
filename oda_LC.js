...
var ios = /(ipad|iphone|ipod)/ig.test( navigator.userAgent ) ? true : false
...
function loadAud( FN, B )
  ...
  function loadPlayer( A )
	...
	if (! ios)
		...
		{
			var b = B ? 'autoplay' : ''
			ws('<audio autoplay="',b,'" controls="true" > ')
			ws('<source src="',A,'" type="audio/mpeg"> ')
			ws('</audio> ')
		}
  ...
  if (! ios)
	...
	{
		loadPlayer(encodeURIComponent(FN))
	}
