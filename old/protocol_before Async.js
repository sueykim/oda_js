...
function server_error_checker(currentSet, url, respond)
    {    
		...
		g_XX.post_form(g_SERVER + 'error_filer.aspx?', 'txt=' + encodeURIComponent(result) + '&url=' + encodeURIComponent(url) + '&lang=' + lang + '&mod=' + (document.fm_track.skill.value=='reading' ? 'RC' : 'LC'))

                //alert('error filer'+g_XX.get_responseText())
            }
    ...
    if(!true)// set true for debugging
      ...
      if (!foundErr)
        ...
        if(levelChk)
          ...
          foundErr = server_error_checker(currentSet, g_SERVER+aspy+ARR.join('&'),  SR)
          //alert('514: Server Response: ' + SR )
          if( ( !foundErr ) && ( SR == 1 ) )
            {
            g_FM.load_form( g_F.length-1, currentSet )
...
function initPg()
  ...
  function loadTest()
	...
	LC = LC +  '&testno=' + testn
      ...
      //==> revert back to this LC = g_SERVER +'ODA_MainAssess.aspx?Tlang=' + lang + '&UID=' + UID + '&lName=' + lName + '&skill=' + skill
